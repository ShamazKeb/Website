...
How it works:
  1. Each iteration feeds a prompt to Gemini via stdin
...
if ! command -v "$GEMINI_CMD" &> /dev/null; then
    echo -e "${RED}Error: Gemini CLI ('gemini') not found${NC}"
    echo ""
    echo "Please ensure the Gemini CLI is installed and in your PATH."
    exit 1
fi
...
Read `CONSTITUTION.md` to understand project principles and constraints.
...
0a. Read `CONSTITUTION.md` for project principles.
...
echo ""
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}               RALPH LOOP (Gemini) STARTING                ${NC}"
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
while true; do
    ...
    # Run Gemini with prompt via stdin, capture output
    GEMINI_OUTPUT=""
    if GEMINI_OUTPUT=$("$GEMINI_CMD" --yolo < "$PROMPT_FILE" 2>&1 | tee "$LOG_FILE"); then
        if [ -n "$WATCH_PID" ]; then
        ...
        if echo "$GEMINI_OUTPUT" | grep -qE "<promise>(ALL_)?DONE</promise>"; then
            ...
            if [ $CONSECUTIVE_FAILURES -ge $MAX_CONSECUTIVE_FAILURES ]; then
                ...
                echo -e "  - Checking the logs in $LOG_DIR${NC}"
                echo -e "  - Simplifying the current spec${NC}"
                echo -e "  - Manually fixing blocking issues${NC}"
                echo ""
                CONSECUTIVE_FAILURES=0
            fi
        fi
